...
def lint(ctx):
    ...
    for exp in resume_data.experiences:
        ...
        for bullet in exp.bullets:
            ...
            if result.violations:
                exp_violations.extend((bullet.text[:60], v) for v in result.violations)
    ...
    if total_violations == 0:
        ...
        click.echo(
            click.style(
                f"⚠ Found {total_violations} violation(s). Fix in data/ or use --strict-filter to auto-redact.",
                fg="yellow",
            )
        )
...
def _apply_content_filters(
    selected: SelectedResume, strict: bool = False
) -> SelectedResume:
